...
C.use_horizontal_flips = bool(options.horizontal_flips)  #横向翻转
C.use_vertical_flips = bool(options.vertical_flips)      #垂直翻转
...
####*********************添加变量**************************########
feature_map_last = (None, 4096)
feature_map_now = (None, 4096)
...
for epoch_num in range(num_epochs):
    ...
    ###%%%%%%%%%%%%%%%%对网络进行训练，先对RPN进行训练，在RPN训练的基础上再训练classifiter网络
    while True:
        try:
            ...
            loss_rpn = model_rpn.train_on_batch(X, Y)   #对RPN进行进行训练

            P_rpn = model_rpn.predict_on_batch(X)   #RPN预测输出
            ...
            #训练classifier网络，采用分步训练的思想
            loss_class = model_classifier.train_on_batch([X, X2[:, sel_samples, :]],
                                                         [Y1[:, sel_samples, :], Y2[:, sel_samples, :]])
            ###*****************************链接faster与GRU*********************###

            feature_map_now = model_classifier.predict(X)
            feature_map_now = GRU.tensor_to_np(feature_map_now)
            min_dist = GRU.dist(feature_map_last[1,:],feature_map_now[1,:])
